function magic_visualization(input_file1, input_file2, output_path, varargin)
   % Inputs:
     ...
     % Outputs
      ...
      % Visualized:
            ...
            % Subnetworks of Hub Genes identified with 3-sigma rule
            % Scatter Plots of the raw correlation under each condition of
                % the gene pair with the highest modulation score
    ...
    for j = 1:2:num_optional_args
        ...
        switch arg_name
            ...
            otherwise
                error(['Unrecognized argument name: ', arg_name]);
        end
    end
    ...
    axis off;
    title("Differential Gene Regulatory Network");
    %% Finding hub genes with 3-sigma rule
    mean_degree = mean(node_degree);
    std_degree = std(node_degree);

    hub_gene_idx  = node_degree>mean_degree + 3*std_degree;
    hub_genes = gene_names(hub_gene_idx);
    
    
    %% Plot subnetworks for hub genes
    for l = 1:numel(hub_genes)
        % find idx of hub gene
        hub_gene_idx = find(strcmp(gene_names, hub_genes{l}));
        ...
        axis off;
        title(['Subnetwork for Hub Gene: ', hub_genes{l}]);
    end
